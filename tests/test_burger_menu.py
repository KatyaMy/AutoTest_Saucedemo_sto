...
from pages.burger_page import BasePage
...
class TestBurgerPage:
    ...
    def test_check_about_bt(self, driver, login_user, wait):
        ...
        driver.find_element(*BurgerMenu.bt_about_us).click()
    ...
    def test_check_reset_app_bt(self, driver, login_user, wait):
        ...
        wait.until(EC.visibility_of_element_located(BurgerMenu.item_list))
        try:
            wait.until(EC.element_to_be_clickable(BurgerMenu.bt_reset_app))
            # если элемент становится кликабельным, продолжает выполнять код
            driver.find_element(*BurgerMenu.bt_reset_app).click()
